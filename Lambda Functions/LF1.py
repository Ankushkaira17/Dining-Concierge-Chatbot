...
    party = slots.get('party').get('value').get('interpretedValue')
    date = slots.get('date').get('value').get('interpretedValue')
    time = slots.get('time').get('value').get('interpretedValue')
    phone = slots.get('phone').get('value').get('interpretedValue')
    email = slots.get('email').get('value').get('interpretedValue')
    
    session_attributes = intent_request['sessionState'] if intent_request['sessionState'] is not None else {}
    
...
    client.send_message(
        QueueUrl='https://sqs.us-east-1.amazonaws.com/653138673154/sqs', 
